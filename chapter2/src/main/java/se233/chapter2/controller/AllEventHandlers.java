    ...
    public static void onAdd() {
        try {
            ...
            if (code.isPresent()) {
                ...
                ArrayList<CurrencyEntity> c_list = FetchData.fetch_range(c.getShortCode().toUpperCase(), 8);
                c.setHistorical(c_list);
                c.setCurrent(c_list.get(c_list.size() - 1));
                currency_list.add(c);
                Launcher.setCurrencyList(currency_list);
                Launcher.refreshPane();
                logger.info("Add currency {}",code.get().toUpperCase());
            }
    ...
    public static void onDelete(String code){
        try{
            ...
            if(index!=-1){
                ...
                Launcher.setCurrencyList(currency_list);
                Launcher.refreshPane();
                logger.info("Delete currency "+code.toUpperCase());
            }
        }catch(InterruptedException e){
            e.printStackTrace();
        }catch(ExecutionException e){
